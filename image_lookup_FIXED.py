...
ФИКС 2026-07-16 (Sherlock): запросы к Wikimedia падали 403 из-за
отсутствия заголовка User-Agent — это требование политики Wikimedia
(https://meta.wikimedia.org/wiki/User-Agent_policy), без него запросы
с датацентровых IP (Render и подобные) режутся. Ошибка проглатывалась
в except -> фото просто никогда не приходило, без единой строки в логах.
"""
import os
...
# Замени контакт на свой (email или ссылку на Telegram/GitHub) — Wikimedia
# просит указывать реальный способ связи в User-Agent, иначе тоже может
# ограничивать запросы.
HEADERS = {
    "User-Agent": "ovqatbot/1.0 (https://github.com/temirovotabek/ovqatbot; "
    "contact: temirovotabek на GitHub) python-requests",
}
